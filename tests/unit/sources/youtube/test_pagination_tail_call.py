"""
Pagination tail-call: when read_table returns end offset {"pageToken": None},
the framework calls again with that offset. Readers must return empty rows, not
re-fetch page 1 (see _read_search pattern).
"""
import sys
...
def test_tail_call_with_page_token_none_returns_empty(mock_connector, table_name, table_options):
    """Second read with end offset must not re-emit page 1."""
    records_iter, end_offset = mock_connector.read_table(table_name, {}, table_options)
    first = list(records_iter)
    assert len(first) > 0
    assert end_offset == {"pageToken": None}

    tail_iter, tail_offset = mock_connector.read_table(table_name, end_offset, table_options)
    tail_records = list(tail_iter)
    assert tail_records == []
    assert tail_offset == {"pageToken": None}
